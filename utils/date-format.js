...
dateformat.getDayOfTheYear = function(timeStamp=new Date().getTime()) {
    let y = new Date(timeStamp).getFullYear();
    let firstDay = new Date(`${y}-01-01 00:00:00`).getTime();
    let day = Math.ceil((timeStamp - firstDay) / 1000 / 24 / 60 / 60);
    return day;
};
dateformat.WEEKTYPE = {
    ZH_DAYNAME: 0,
